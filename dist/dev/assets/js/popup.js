  ...
  shorten: function (page) {
    ...
    try {
      urlObject = new URL(page.url);
			urlIsValid = (urlObject.protocol === 'http:' || urlObject.protocol === 'https:');
    } catch (e) {
      console.error(e);
    }

    if ((urlIsValid || Func.urlIsValid(page.url)) && (page.url).toLowerCase().indexOf('shly.link') === -1) {
			// !['shly.link'].includes(urlObject.domain)
